...
func NewMysqlConnection(connectionProfile ConnectionProfile) *MysqlConnection {
	return &MysqlConnection{connectionProfile}
}

func (mc *MysqlConnection) ConnectDb() (*gorm.DB, error) {
...
func (mc *MysqlConnection) SyncSchema(targetDB *gorm.DB) error {
	var err error
	tables, err := mc.getMysqlTableList(targetDB)
	if err != nil {
		return err
	}
	if mc.getMysqlDescribeTable(targetDB, tables) != nil {
		log.Println("######## Failed to describe Mysql tables ########\n", err.Error())
		return err
	}
	return nil
}

func (mc *MysqlConnection) getMysqlTableList(targetDB *gorm.DB) ([]string, error) {
	var tables []string
	if result := targetDB.Raw("SHOW TABLES").Scan(&tables); result.Error != nil {
		log.Println("######## Failed to get mysql tables from target database ########\n", result.Error.Error())
		return nil, result.Error
	}
	return tables, nil
}

func (mc *MysqlConnection) getMysqlDescribeTable(targetDB *gorm.DB, tables []string) error {
